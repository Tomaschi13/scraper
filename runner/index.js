...
//
// These defaults are used when no explicit override is passed and either
// no proxy is configured or the geo probe fails. With --auto-stealth-geo
// (and a proxy), runtime resolution overrides them with values coherent
// to the proxy's egress country.
const STEALTH_LOCALE = "en-US";
const STEALTH_TIMEZONE = "Europe/Vilnius";

// Country-code → BCP-47 locale that a real resident of that country is
// most likely to advertise. Kept deliberately small; missing entries fall
// back to en-US which is the safe global default. Used only when
// --auto-stealth-geo is enabled and the proxy probe succeeds.
const COUNTRY_LOCALE_MAP = {
  AT: "de-AT", AU: "en-AU", BE: "nl-BE", BG: "bg-BG", BR: "pt-BR",
  CA: "en-CA", CH: "de-CH", CZ: "cs-CZ", DE: "de-DE", DK: "da-DK",
  EE: "et-EE", ES: "es-ES", FI: "fi-FI", FR: "fr-FR", GB: "en-GB",
  GR: "el-GR", HR: "hr-HR", HU: "hu-HU", IE: "en-IE", IT: "it-IT",
  JP: "ja-JP", KR: "ko-KR", LT: "lt-LT", LU: "fr-LU", LV: "lv-LV",
  MX: "es-MX", NL: "nl-NL", NO: "nb-NO", NZ: "en-NZ", PL: "pl-PL",
  PT: "pt-PT", RO: "ro-RO", RS: "sr-RS", SE: "sv-SE", SI: "sl-SI",
  SK: "sk-SK", TR: "tr-TR", UA: "uk-UA", US: "en-US"
};
...
const EGRESS_PROBE_PATH = "/json?fields=status,country,countryCode,timezone,query";
...
function usage() {
  return [
    ...
    "  --auto-stealth-geo   Derive locale/timezone from the proxy egress IP (default: false). Explicit overrides above always win.",
...
// Probes the proxy's egress geolocation by issuing an HTTP request through
// the proxy itself. Used both for the IP-repetition guard and for deriving
// a locale/timezone identity coherent with the exit IP's country.
//
// Only HTTP/HTTPS proxies are supported; SOCKS proxies need a different
// transport so we return null and let the caller fall back to defaults.
// Falls back to a plain HTTP request (not HTTPS-via-CONNECT) because that
// is the simplest reliable path and the response carries no secrets.
...
// Resolves the (locale, timezone) tuple to advertise to cloakbrowser for
// this launch. Precedence:
//   1. explicit overrides (--stealth-locale / --stealth-timezone)
//   2. proxy-geo derivation (when --auto-stealth-geo and a probe result)
//   3. hardcoded module defaults (en-US / Europe/Vilnius)
// The proxy probe is a *suggestion* — explicit flags always win, so an
// operator can pin a known-good identity for a given target site even
// when the proxy egress moves between runs.
function resolveStealthIdentity({ probe, override, autoGeo }) {
  const explicitLocale = String(override?.locale || "").trim();
  const explicitTimezone = String(override?.timezone || "").trim();

  if (explicitLocale && explicitTimezone) {
    return {
  ...
  if (autoGeo && probe?.countryCode && probe?.timezone) {
    const mappedLocale = COUNTRY_LOCALE_MAP[probe.countryCode] || "en-US";
    return {
      locale: explicitLocale || mappedLocale,
      timezone: explicitTimezone || probe.timezone,
      source: explicitLocale || explicitTimezone ? "override+proxy-geo" : "proxy-geo"
    };
  }

  return {
    locale: explicitLocale || STEALTH_LOCALE,
    timezone: explicitTimezone || STEALTH_TIMEZONE,
    source: explicitLocale || explicitTimezone ? "override+default" : "default"
  };
}
...
async function run() {
  ...
  try {
    ...
    // Probe the proxy egress before launch so we can (a) detect IP repetition
    // against recent runs of this robot and (b) optionally derive a locale +
    // timezone identity coherent with the egress country. The probe is
    // best-effort: if it fails (timeout, SOCKS proxy, etc.) we still launch
    // with the explicit/default identity.
    let egressProbe = null;
    if (config.proxy) {
      egressProbe = await probeProxyEgress(config.proxy);
      if (egressProbe) {
        console.log(`Proxy egress: ${egressProbe.ip} (${egressProbe.countryCode || "??"} / ${egressProbe.timezone || "??"})`);  // eslint-disable-line no-console
        ...
        console.log("Proxy egress probe failed; falling back to explicit/default stealth identity.");  // eslint-disable-line no-console
      }
    }

    const stealthIdentity = resolveStealthIdentity({
      probe: egressProbe,
      override: { locale: config.stealthLocale, timezone: config.stealthTimezone },
      autoGeo: config.autoStealthGeo
    });
    console.log(`Stealth identity: locale=${stealthIdentity.locale} timezone=${stealthIdentity.timezone} (source=${stealthIdentity.source})`);  // eslint-disable-line no-console
    ...
    context = await cb.launchPersistentContext({
      ...
      locale: stealthIdentity.locale,
      timezone: stealthIdentity.timezone,
